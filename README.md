...
Every sighting is a Markdown file in `src/content/sightings/`, with a screenshot alongside it. The schema in `src/content.config.ts` is the single source of truth for what's tracked — Astro validates every entry against it at build time, so a malformed entry fails the build rather than shipping broken.

## Adding a sighting

**As a PR:** drop a `.md` file in `src/content/sightings/` and its screenshot in `src/content/sightings/images/`. Copy `example-movie-2024.md` as a starting point. Open a PR.
